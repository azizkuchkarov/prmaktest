...
/** 6-sinf blok: kabinetda 5–9 sinflar bitta leaderboardda (faqat ball yig‘ganlar). */
export const LEADERBOARD_MIDDLE_GRADE_MIN = 5;
export const LEADERBOARD_MIDDLE_GRADE_MAX = 9;
export const LEADERBOARD_PRIMARY_REPUBLIC_GRADE = 4;

export async function getMiddleGradesRepublicLeaderboard(limit: number): Promise<LeaderboardRow[]> {
  const gMin = LEADERBOARD_MIDDLE_GRADE_MIN;
  const gMax = LEADERBOARD_MIDDLE_GRADE_MAX;
  const rows = await prisma.$queryRaw<
    {
      rank: bigint;
      userId: string;
  ...
  WITH sub AS (
    ...
    FROM "TestAttempt" ta
    INNER JOIN "User" u ON u."id" = ta."userId"
    WHERE u."gradeLevel" >= ${gMin} AND u."gradeLevel" <= ${gMax}
    GROUP BY ta."userId"
    HAVING SUM(ta."rankPoints") > 0
  ),
  ranked AS (
...
export async function getMiddleGradesViloyatLeaderboard(viloyat: string, limit: number): Promise<LeaderboardRow[]> {
  const gMin = LEADERBOARD_MIDDLE_GRADE_MIN;
  const gMax = LEADERBOARD_MIDDLE_GRADE_MAX;
  const rows = await prisma.$queryRaw<
    {
  ...
  WITH sub AS (
    ...
    INNER JOIN "User" u ON u."id" = ta."userId"
    WHERE u."viloyat" = ${viloyat}
      AND u."gradeLevel" >= ${gMin} AND u."gradeLevel" <= ${gMax}
    GROUP BY ta."userId"
    HAVING SUM(ta."rankPoints") > 0
  ),
  ranked AS (
...
export async function getStudentRankSummary(
  ...
  let gradeRepublicRank: number | null = null;
  let gradeViloyatRank: number | null = null;

  if (gl === LEADERBOARD_PRIMARY_REPUBLIC_GRADE) {
  ...
  } else if (gl >= LEADERBOARD_MIDDLE_GRADE_MIN && gl <= LEADERBOARD_MIDDLE_GRADE_MAX) {
    const gMin = LEADERBOARD_MIDDLE_GRADE_MIN;
    const gMax = LEADERBOARD_MIDDLE_GRADE_MAX;

    const higherMidRep = await prisma.$queryRaw<[{ c: bigint }]>`
    SELECT COUNT(*)::bigint AS c
    FROM (
      SELECT
        ta."userId",
  ...
  `;
    ...
    const higherMidVil = await prisma.$queryRaw<[{ c: bigint }]>`
    SELECT COUNT(*)::bigint AS c
    FROM (
      SELECT
        ta."userId",
      ...
      FROM "TestAttempt" ta
      INNER JOIN "User" u ON u."id" = ta."userId"
      WHERE u."viloyat" = ${viloyat} AND u."gradeLevel" >= ${gMin} AND u."gradeLevel" <= ${gMax}
      GROUP BY ta."userId"
      HAVING SUM(ta."rankPoints") > 0
    ) o
    WHERE o."userId" != ${userId}
  ...
  `;
    ...
    gradeRepublicRank = (higherMidRep[0] != null ? Number(higherMidRep[0].c) : 0) + 1;
    gradeViloyatRank = (higherMidVil[0] != null ? Number(higherMidVil[0].c) : 0) + 1;
  }
